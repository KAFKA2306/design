...
- `registry/ui/product-ui.tsx` is the stable Product UI public entrypoint. Product UI implementation is split by responsibility under `registry/ui/product/`; do not move implementations back into the entrypoint.
- Product charts use the centrally configured Recharts adapter in `registry/ui/product/chart.tsx`; consumers must not redefine palette, grid, line geometry, actual/forecast styling, tooltip, or chart spacing.
